...
export default function TrendingSection({
  ...
  const handleHashtagClick = (hashtag: string) => {
    if (onHashtagClick) {
      ...
      navigate({
        to: "/topic/$topic",
        params: { topic: encodeURIComponent(hashtag) },
      });
    }
  };
    ...
    <div className="bg-white border border-border rounded-lg p-4 shadow-sm mb-4">
      ...
      {!trendingHashtags || trendingHashtags.length === 0 ? (
        ...
        <ScrollArea className="w-full whitespace-nowrap">
          <div
            className={`flex gap-2 pb-2 transition-opacity duration-500 ${isRefreshing ? "animate-shimmer" : ""}`}
          >
            {trendingHashtags.map((hashtag) => (
              <Button
                key={hashtag.word}
                variant={
                  selectedHashtag === hashtag.word ? "default" : "outline"
                }
                size="sm"
